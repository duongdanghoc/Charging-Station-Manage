...
export const StationPinTool: React.FC<StationPinToolProps> = ({
  ...
  const [pendingStation, setPendingStation] = useState<{ lat: number; lng: number } | null>(null);
  ...
  const handleSaveStation = async () => {
    ...
    try {
      ...
      onStationAdded(savedStation); // callback cập nhật danh sách trạm ở client
      handleCancelPin();
    } catch (error) {
  ...
  return (
    <>
      ...
      <button
        onClick={onToggle}
        className={`inline-flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-offset-1 ${
          isActive
          ...
          <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 px-4 py-8">
            <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Nhấp lên bản đồ để chọn vị trí, sau đó điền thông tin chi tiết cho trạm.
                  </p>
                </div>
                <button
              ...
              <div className="mt-5 space-y-4">
                {/* Name */}
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">Tên trạm *</label>
                  <input
                    type="text"
                    value={formData.name}
                ...
                {/* Type */}
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">Loại trạm</label>
                  <div className="flex gap-3 text-sm">
                    <label
                      className={`flex items-center gap-2 rounded-lg border px-3 py-2 ${
                        formData.type === "rescue"
                ...
                {/* Contact */}
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">Liên hệ</label>
                  <input
                    type="text"
                ...
                <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-600">
                  ...
                  <span className="font-semibold text-slate-800">
                    {pendingStation.lat.toFixed(6)}, {pendingStation.lng.toFixed(6)}
                  </span>
                </div>
              </div>
